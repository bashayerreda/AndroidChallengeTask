...
fun MutableList<String>.fetchWords() : MutableList<WordsModel>{
    if(this.isEmpty()){
        return mutableListOf()
    }else{
        val list: MutableList<WordsModel> = mutableListOf()
        ...
        for (c in map.keys) {
            if (map[c] != null)
            list.add(WordsModel(name = c, quantity = map[c]!!))
        }
        return list
    }
}
...
fun String.parseHTML(): String {
    return if(this.isEmpty()){
        ...
        matcher.replaceAll("").toLowerCase()
    }
